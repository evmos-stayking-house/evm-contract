...
export const deployMockSwap = async (
    deployer: SignerWithAddress,
    Tokens: ERC20OwnableCraft[],
    evmos_supply: number
) => {
    const MockSwap = await craftform.contract('MockSwap').deploy(null, {
        from: deployer.address,
        args: [[ZERO_ADDRESS, ...Tokens.map((t) => t.address)]],
    });

    const MockSwapHelper = await craftform
    ...
    if (token0balance.lte(toBN(1, 18))) {
        // less than 1 ETH
