...
describe('EVMOS Hackathon Test', async () => {
    ...
    before(async function () {
        await deployLocal();
        [deployer, delegator, lender1, staker1, validator, killer] =
    ...
    describe('1. Vault:: initial deployed', async function () {
        ...
        it('First time deposit & withdraw', async function () {
            ...
            const [step0Amount, step0Share] = await getBalances(
                lender1.address
            );
            ...
            const [step2Amount, step2Share] = await getBalances(
                lender1.address
            );
            ...
            // after Step 1 balance status
            // at initial deposit, USDC : ibUSDC ratio = 1
            const [step3Amount, step3Share] = await getBalances(
                lender1.address
            );
    ...
    describe('2. StayKing:: Add/Change position', async function () {
        it('First add position (leverage x3)', async function () {
            ...
            expect(beforeEVMOS.sub(afterEVMOS)).to.approximately(
                equity,
                toBN(1, 16)
            );

            const positionId = await Stayking.positionIdOf(
                staker1.address,
                ibtUSDC.address
            );
            expect(positionId).to.eq(1);
            const [positionValueInBase, positionDebtInBase, debt] =
                await Stayking.positionInfo(staker1.address, tUSDC.address);

            // equity in position
            expect(positionValueInBase.sub(positionDebtInBase)).to.equal(
                equity
            );
        ...
        it('accrued interest on next day is valid', async function () {
            ...
            // => x/3 : interest rate
            const expected = expectedIR
                .mul(totalStakedDebt.add(beforeAccInterest))
                .mul(86400)
                .div(toBN(1, 18));
        ...
        it('Add Equity', async function () {
            ...
            const [afterPosVaule, afterDebtInBase] =
                await Stayking.positionInfo(staker1.address, tUSDC.address);
            ...
            expect(mockValidator.amount).to.equal(
                beforeTotalStaked.add(extraEquity)
            );
        });
        ...
        it('Add Debt', async function () {
            ...
            // position value should be increased
            expect(afterPosVaule.sub(beforePosVaule)).to.equal(
                extraDebtInBase,
                'Position value not increased properly'
            );
            // debt should be increased
            expect(afterDebtInBase.sub(beforeDebtInBase)).to.equal(
                extraDebtInBase,
                'Debt value not increased properly'
            );
            ...
            expect(mockValidator.amount).to.equal(
                beforeTotalStaked.add(extraDebtInBase)
            );
        });
        ...
        it('returns utilization rate properly.', async function () {
            ...
            expect(userDebt.mul(1e4).div(totalLended.add(userDebt))).to.equal(
                ur
            );
        });
    });
    ...
    describe('3. Lock uEVMOS', async function () {
        it('Remove Position', async function () {
             ...
             * Stayking check
             */
            const tx = await Stayking.connect(staker1).removePosition(
                tUSDC.address
            );
            await mockValidator.handleTx(tx);
            ...
            const [afterPosVaule, afterDebtInBase] =
                await Stayking.positionInfo(staker1.address, tUSDC.address);
            const pendingDebt = await ibtUSDC.getPendingDebtInBase(
                staker1.address
            );
        ...
        it('should unlock after 14(+2) days', async function () {
            const beforeUnlockable = await uEVMOS.getUnlockable(
                staker1.address
            );
            ...
            const expectedDebtInBase = await ibtUSDC.getBaseOut(
                locked.debtShare
            );
        ...
        it('EVOMS should unlocked except debt', async function () {
            ...
            expect(afterStakerBalance).to.be.approximately(
                beforeStakerBalance.add(
                    unlockable.unlockable.sub(unlockable.debt)
                ),
    ...
    describe('4. Add/Handle position after first time', async function () {
        it('Re-add position (x3 leverage)', async function () {
            ...
            expect(mockValidator.amount).to.equal(
                beforeStakedAmount.add(positionValueInBase)
            );
    ...
    describe('5. Interest Moel: Triple Slope Model', async function () {
        ...
        it('TripleSlopeModel Case 1 : debt ratio = 0%', async function () {
            const maybe0 = await model.calcInterestRate(0, toBN(100, 24));
    ...
    describe('6. Delegator accrues when auto-compounding', async function () {
        it('mockValidator works well', async function () {
            ...
            expect(afterClaimDelegatorBalance).to.equal(
                beforeDelegatorBalance.add(claimed)
            );
        ...
        it('accrues interest to vault properly.', async function () {
            ...
            const tx = await Stayking.connect(delegator).accrue(
            ...
            expect(await Stayking.totalAmount()).to.equal(mockValidator.amount);
            expect(mockValidator.amount.sub(beforeStaked)).to.approximately(
                expectedAccrued,
                toBN(1, 4)
            );
        });
        ...
        it("After accrued, every position's debt ratio should be decreased", async function () {
            const [beforePositionValueInBase, beforePositionDebtInBase] =
                await Stayking.positionInfo(staker1.address, tUSDC.address);
            const beforeDebtRatio = beforePositionDebtInBase
                .mul(1e4)
                .div(beforePositionValueInBase);

            const claimed = await mockValidator.claim();
            await Stayking.connect(delegator).accrue(
                mockValidator.amount.add(claimed),
                { value: claimed }
            );

            const [afterPositionValueInBase, afterPositionDebtInBase] =
                await Stayking.positionInfo(staker1.address, tUSDC.address);
            const afterDebtRatio = afterPositionDebtInBase
                .mul(1e4)
                .div(afterPositionValueInBase);
    ...
    describe('7. Advanced change position', async function () {
        ...
        it('Case 2: Remove equity', async function () {
            // check before status
            const [beforePositionValue, beforePositionDebt] =
                await Stayking.positionInfo(staker1.address, tUSDC.address);
            ...
            expect(afterTotalStaked).to.equal(
                beforeTotalStaked.sub(removedEquity)
            );
        });
        ...
        it('Case 3: Add equity, Add debt', async function () {
            ...
            const [afterPositionValue, afterPositionDebt] =
                await Stayking.positionInfo(staker1.address, tUSDC.address);
            expect(afterPositionDebt).to.equal(
                beforePositionDebt.add(extraDebtInBase)
            );
            expect(afterPositionValue).to.equal(
            ...
            const afterTotalStaked = mockValidator.amount;
            expect(afterTotalStaked).to.equal(
                beforeTotalStaked.add(extraDebtInBase).add(extraEquityInBase)
            );
        });
        ...
        it('Case 7: Remove equity, Remove debt', async function () {
            ...
            const [afterPositionValue, afterPositionDebt] =
                await Stayking.positionInfo(staker1.address, tUSDC.address);
            expect(afterPositionDebt).to.equal(
                beforePositionDebt.sub(removedDebt)
            );
            expect(afterPositionValue).to.equal(
            ...
            const afterTotalStaked = mockValidator.amount;
            expect(afterTotalStaked).to.equal(
                beforeTotalStaked.sub(removedDebt).sub(removedEquity)
            );

            // check uEVMOS
            const afterLocked = await uEVMOS.balanceOf(staker1.address);
            expect(afterLocked).to.equal(
                beforeLocked.add(removedEquity).add(removedDebt)
            );
        });
    });
    ...
    describe('8. Position with bad debt should be killed.', async function () {
        ...
        it("As EVMOS price changes, position's debt ratio get increased and killable state may changed.", async function () {
            ...
            const beforeKillable = await Stayking.isKillable(
                tUSDC.address,
                positionId
            );
            ...
            const afterKillable = await Stayking.isKillable(
                tUSDC.address,
                positionId
            );
        ...
        it('Position should be killed', async function () {
            ...
            // pended debt
            expect(pendedDebt).to.approximately(beforePositionDebt, toBN(1, 4));
