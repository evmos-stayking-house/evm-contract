...
describe('Testnet fork test', async () => {
    describe('컨트랙트', function () {
        ...
        it('EvmoSwapHelper 배포', async function () {
            ...
            const EvmoSwapHelper = await craftform
        ...
        it('getDx', async function () {
            const EvmoSwap = await craftform
                .contract('EvmoSwapRouter')
                .attach();
            const EvmoSwapHelper = await craftform
                .contract('EvmoSwapHelper')
                .attach();
            ...
            const dy = toBN(1, 6);
            const swapDx = await EvmoSwap.getAmountsIn(dy, [
        ...
        it('getDy', async function () {
            ...
            const dx = toBN(1, 18);
            const swapDy = await EvmoSwap.getAmountsOut(dx, [
                TOKEN_ADDRESS.mockWEVMOS,
                TOKEN_ADDRESS.mockUSDT,
            ]);
        ...
        it('exchange', async function () {
            ...
            expect(beforeEVMOS.sub(afterEVMOS)).to.equal(
                dx.add(gasUsed.mul(tx.gasPrice!))
            );
        });
