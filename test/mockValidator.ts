...
export class MockValidator {
    ...
    async claim(){
        ...
        await setBalance(this.delegator.address, beforeDelegatorBalance.add(claimAmount));
        return claimAmount;
    }
    ...
    async unbond(){
        ...
        await this.uEVMOS.connect(this.delegator).supplyUnbondedToken({value: unbonded})
